...
const People = ({ people, containerDimensions }: any) => {
    ...
    return (
        <View style={peopleStyles.container}>
            {visiblePeople?.map((person: any, index: number) => {
                return (
                    <View key={index} style={[peopleStyles.card, person?.speaking ? {
                        borderWidth: 3,
                    } : null,
                    Array.isArray(gridStyle) ? gridStyle[index] : gridStyle
                        ...
                        <Text style={peopleStyles.name}>{person?.name}</Text>
                        {
                            !person?.micOn && (
                                <View style={peopleStyles.muted}>
                                    <MicOff size={RFValue(10)} color={"#fff"} />
                                </View>
                            )
                        }
                        ...
                        </View>
                        {
                            othersCount > 0 && index==visiblePeople?.length-1 && (
                                <TouchableOpacity style={[peopleStyles.others]}
                                activeOpacity={0.8}
                                >
...
const getGridStyle = (count: number, containerWidth: number, containerHeight: number) => {
